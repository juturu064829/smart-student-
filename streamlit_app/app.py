...
def get_connection():
    if not os.path.exists(DB_PATH):
        st.error(f"Database file not found at {DB_PATH}. Please run `python database/seed_data.py` first.")
        st.stop()
...
selected_dept_code = st.sidebar.selectbox("Filter Department", ["All"] + list(depts_df['code'].unique()))
selected_course_code = st.sidebar.selectbox("Filter Course", ["All"] + list(courses_df['code'].unique()))
